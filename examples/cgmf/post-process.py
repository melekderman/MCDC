...
with h5py.File("output_cgmf.h5", "r") as f1:
    E_cgmf = f1["tallies/flux_TD/grid/energy"][:]
    t_cgmf = f1["tallies/flux_TD/grid/time"][:]
    phi_cgmf = f1["tallies/flux_TD/flux/mean"][:]  # shape: (Nt, NE)
    sd_cgmf = f1["tallies/flux_TD/flux/sdev"][:]

with h5py.File("output_endf.h5", "r") as f2:
    E_endf = f2["tallies/flux_TD/grid/energy"][:]
    t_endf = f2["tallies/flux_TD/grid/time"][:]
    phi_endf = f2["tallies/flux_TD/flux/mean"][:]  # shape: (Nt, NE)
    sd_endf = f2["tallies/flux_TD/flux/sdev"][:]

E_mid_cgmf = 0.5 * (E_cgmf[:-1] + E_cgmf[1:])
E_mid_endf = 0.5 * (E_endf[:-1] + E_endf[1:])
t_mid_cgmf = 0.5 * (t_cgmf[:-1] + t_cgmf[1:])
t_mid_endf = 0.5 * (t_endf[:-1] + t_endf[1:])
...
print("phi_endf shape:", phi_endf.shape)
...
(line_m,) = ax.plot([], [], "b-", lw=1.5, label="ENDF")
fill_m = ax.fill_between([], [], [], alpha=0.2, color="b")
time_text = ax.set_title("")
ax.legend()

# Set y limits from full dataset
ymin = min(phi_cgmf[phi_cgmf > 0].min(), phi_endf[phi_endf > 0].min()) * 0.5
ymax = max(phi_cgmf.max(), phi_endf.max()) * 2.0
...
phi_t_endf = phi_endf.sum(axis=0)
phi_t_sd_endf = np.sqrt((sd_endf**2).sum(axis=0))

ax0.plot(t_mid_cgmf * 1e9, phi_t_cgmf, "r-", alpha=0.6, label="CGMF")
ax0.plot(t_mid_endf * 1e9, phi_t_endf, "b-", alpha=0.6, label="ENDF")
ax0.set_xlabel("$t$ [ns]")
...
# ── Animate ───────────────────────────────────────────────────────
n_frames = min(len(t_mid_cgmf), len(t_mid_endf))
...
def animate(i):
    ...
    line_m.set_data(E_mid_endf, ym)

    # Redraw fill_between
    fill_c.remove()
    fill_m.remove()
    fill_c = axes[1].fill_between(
        E_mid_cgmf, yc - sd_cgmf[:, i], yc + sd_cgmf[:, i], alpha=0.2, color="r"
    )
    fill_m = axes[1].fill_between(
        E_mid_endf, ym - sd_endf[:, i], ym + sd_endf[:, i], alpha=0.2, color="b"
    )
    ...
    marker_m.set_data([t_mid_endf[i] * 1e9], [phi_t_endf[i]])
...
anim.save("cgmf_vs_endf_spectrum.gif", writer="pillow", fps=15)
